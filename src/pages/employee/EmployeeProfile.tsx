...
import { User, Mail, Calendar, Loader2, Save, Key, CheckCircle, Eye, EyeOff } from 'lucide-react';
...
export default function EmployeeProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const { data } = await supabase.from('employees').select('first_name, last_name, email, photo_url').eq('id', employeeId).maybeSingle();
      if (data) {
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 -mt-16">
              ...
              <div className="text-center sm:text-left flex-1 space-y-2">
                <h2 className="text-2xl font-bold text-foreground">
                  {profile.first_name && profile.last_name ? `${profile.first_name} ${profile.last_name}` : 'Employee'}
                </h2>
