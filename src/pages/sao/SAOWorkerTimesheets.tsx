...
export default function SAOWorkerTimesheets() {
  ...
  const fetchWorkerAndTimesheets = async () => {
    // Fetch worker without SAO restriction first
    const { data: workerData, error: workerError } = await supabase
      .from('workers')
      .select('id, worker_id, first_name, last_name, photo_url, hourly_rate, assigned_sao_id')
      .eq('id', workerId)
      .single();
    ...
    // Verify this SAO has access: legacy assigned_sao_id OR junction row
    let hasAccess = workerData.assigned_sao_id === employeeId;
    if (!hasAccess) {
