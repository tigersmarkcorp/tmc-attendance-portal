...
import { WorkerAttendanceLog } from '@/components/sao/WorkerAttendanceLog';
import { 
  Loader2, 
  Search, 
  HardHat, 
...
export default function SAOWorkers() {
  ...
  useEffect(() => {
    if (employeeId) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'worker_sao_assignments' }, () => fetchAssignedWorkers())
  ...
  const fetchAssignedWorkers = async () => {
    // Junction-based assignments (multi-SAO)
    const { data: junction } = await supabase
      .from('worker_sao_assignments')
      .select('worker_id')
      .eq('sao_employee_id', employeeId);

    const junctionIds = junction?.map(j => j.worker_id) ?? [];

    // Combine legacy assigned_sao_id + junction worker ids
    const orFilter = junctionIds.length > 0
      ? `assigned_sao_id.eq.${employeeId},id.in.(${junctionIds.join(',')})`
      : `assigned_sao_id.eq.${employeeId}`;
            ...
            <>
              ...
              <Tabs defaultValue="personal" className="w-full">
                ...
                <TabsContent value="attendance" className="mt-4">
                  <WorkerAttendanceLog workerId={selectedWorker.id} />
                </TabsContent>
