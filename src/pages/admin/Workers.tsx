...
import { AssignMultipleSAOsDialog } from '@/components/admin/AssignMultipleSAOsDialog';
import { MultiSAOBadges } from '@/components/admin/MultiSAOBadges';
...
export default function Workers() {
  ...
  const [weeklyTimeWorker, setWeeklyTimeWorker] = useState<Worker | null>(null);
  const [payrollSheetOpen, setPayrollSheetOpen] = useState(false);
  const [payrollWorker, setPayrollWorker] = useState<Worker | null>(null);
  const [assignSAOOpen, setAssignSAOOpen] = useState(false);
  const [workerToAssignSAO, setWorkerToAssignSAO] = useState<Worker | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('admin-workers-realtime')
  ...
  return (
    <DashboardLayout title="Workers Management">
      <div className="space-y-6">
              ...
              <CarouselContent className="-ml-2">
                {filteredWorkers.map((worker) => (
                  <CarouselItem key={`card-${worker.id}`} className="pl-2 basis-1/3 sm:basis-1/4 md:basis-1/5 lg:basis-1/6 xl:basis-[12.5%]">
                    <Card
                      className="group cursor-pointer border border-border hover:border-amber-500/40 hover:shadow-xl transition-all duration-300 relative overflow-hidden"
                      ...
                      <div className="relative w-full aspect-[3/4] bg-muted overflow-hidden">
                        {worker.photo_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-500/20 via-orange-500/10 to-amber-600/20">
                            <span className="text-2xl font-black text-amber-500/40 select-none">
                        ...
                        <div className="absolute top-1 right-1 z-10" onClick={(e) => e.stopPropagation()}>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              <DropdownMenuItem onClick={() => { setDetailWorker(worker); setDetailSheetOpen(true); }}>
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => handleStatusChange(worker.id, worker.status === 'active' ? 'inactive' : 'active')}>
                                {worker.status === 'active' ? <XCircle className="w-3 h-3 mr-2" /> : <CheckCircle className="w-3 h-3 mr-2" />}
                                {worker.status === 'active' ? 'Set Inactive' : 'Set Active'}
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-1.5 text-white">
                          ...
                          <p className="text-[8px] text-white/70 truncate">{worker.position || 'No position'}</p>
                        </div>
                      </div>
                    </Card>
                  </CarouselItem>
                    ...
                    {filteredWorkers.map((worker) => {
                      ...
                      return (
                        <TableRow key={worker.id} className="group hover:bg-amber-500/5 transition-colors">
                          ...
                          <TableCell>
                            <DailyTimeDisplay entityType="worker" entityId={worker.id} showLabel />
                          </TableCell>
                          <TableCell>
                            <button
                              type="button"
                              onClick={() => { setWorkerToAssignSAO(worker); setAssignSAOOpen(true); }}
                              className="text-left hover:opacity-80 transition-opacity"
                              title="Click to assign SAOs"
                            >
                              <MultiSAOBadges workerId={worker.id} legacySaoId={worker.assigned_sao_id} />
                            </button>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-52">
                                ...
                                <DropdownMenuItem onClick={() => { setWorkerToAssignSAO(worker); setAssignSAOOpen(true); }}>
                                  <UserCog className="w-4 h-4 mr-2 text-amber-500" />
                                  Assign SAOs
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => { setWeeklyTimeWorker(worker); setWeeklyTimeOpen(true); }}>
        ...
        {/* Assign Multiple SAOs Dialog */}
        {workerToAssignSAO && (
          <AssignMultipleSAOsDialog
            open={assignSAOOpen}
            onOpenChange={setAssignSAOOpen}
            workerId={workerToAssignSAO.id}
            workerName={`${workerToAssignSAO.first_name} ${workerToAssignSAO.last_name}`}
            onSuccess={fetchWorkers}
          />
        )}
        {/* Worker Detail Sheet */}
        ...
        {weeklyTimeWorker && (
          <WeeklyTimeBreakdown
            ...
            entityType="worker"
            entityId={weeklyTimeWorker.id}
            entityName={`${weeklyTimeWorker.first_name} ${weeklyTimeWorker.last_name}`}
          />
        )}
