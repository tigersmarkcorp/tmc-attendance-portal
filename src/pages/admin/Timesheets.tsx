...
import { FileText, Search, DollarSign, Clock, CheckCircle, Loader2, Trash2 } from 'lucide-react';
...
export default function Timesheets() {
  ...
  const handleDeleteTimesheet = async () => {
    if (!deleteId) return;
    setDeleting(true);
    const { error } = await supabase.from('timesheets').delete().eq('id', deleteId);
    if (error) {
      toast({ title: 'Error', description: 'Failed to delete timesheet', variant: 'destructive' });
    } else {
      toast({ title: 'Deleted', description: 'Timesheet deleted successfully' });
      fetchTimesheets();
    }
    setDeleting(false);
    setDeleteId(null);
