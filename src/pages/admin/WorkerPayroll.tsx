...
export default function WorkerPayroll() {
  ...
  const dailyRate = (worker?.hourly_rate || 0) * 8;

  const handleDownloadExcel = () => {
    if (!worker || timesheets.length === 0) return;
    const hourlyRate = worker.hourly_rate || 0;

    const rows: string[][] = [];
    rows.push(['WORKER PAYROLL REPORT']);
    rows.push(['Worker', `${worker.first_name} ${worker.last_name}`]);
    rows.push(['Worker ID', worker.worker_id]);
    rows.push(['Pay Period', selectedPeriod.label]);
    rows.push(['Daily Rate', dailyRate.toFixed(2)]);
    rows.push(['Hourly Rate', hourlyRate.toFixed(2)]);
    rows.push([]);
    rows.push(['Date', 'Day', 'Time In', 'Time Out', 'Total Hours', 'Regular Hours', 'OT Hours', 'Regular Pay', 'OT Pay', 'Total Pay']);
    ...
    timesheets.forEach((ts) => {
      ...
      rows.push([
        ...
        ((ts.total_work_minutes || 0) / 60).toFixed(2),
        (ts.regular_hours || 0).toFixed(2),
        (ts.overtime_hours || 0).toFixed(2),
        (ts.regular_pay || 0).toFixed(2),
        (ts.overtime_pay || 0).toFixed(2),
        (ts.total_pay || 0).toFixed(2),
      ]);
    });

    rows.push([]);
    rows.push(['TOTALS', '', '', '', totals.totalHours.toFixed(2), totals.regularHours.toFixed(2), totals.overtimeHours.toFixed(2), totals.regularPay.toFixed(2), totals.overtimePay.toFixed(2), totals.totalPay.toFixed(2)]);
    rows.push([]);
    rows.push(['SUMMARY']);
    rows.push(['Daily Rate', dailyRate.toFixed(2)]);
    rows.push(['Hourly Rate', hourlyRate.toFixed(2)]);
    rows.push(['Gross Regular Pay', totals.regularPay.toFixed(2)]);
    rows.push(['Gross Overtime Pay', totals.overtimePay.toFixed(2)]);
    rows.push(['Total Pay', totals.totalPay.toFixed(2)]);
    ...
    link.href = url;
    const safeName = `${worker.first_name}_${worker.last_name}`.replace(/[^a-zA-Z0-9]/g, '_');
    const safePeriod = selectedPeriod.label.replace(/[^a-zA-Z0-9]/g, '_');
    link.download = `Worker_Payroll_${safeName}_${safePeriod}.csv`;
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="p-4 rounded-lg bg-muted/50">
                ...
                <p className="text-xl font-bold">{formatCurrency(dailyRate)}</p>
                <p className="text-xs text-muted-foreground mt-1">8 hrs × {formatCurrency(worker.hourly_rate || 0)}</p>
              </div>
              <div className="p-4 rounded-lg bg-muted/50">
                <p className="text-sm text-muted-foreground">Hourly Rate</p>
                <p className="text-xl font-bold">{formatCurrency(worker.hourly_rate || 0)}</p>
              </div>
              <div className="p-4 rounded-lg bg-muted/50">
                <p className="text-sm text-muted-foreground">Gross Regular Pay</p>
                <p className="text-xl font-bold">{formatCurrency(totals.regularPay)}</p>
                <p className="text-xs text-muted-foreground mt-1">{totals.regularHours.toFixed(1)} hrs × {formatCurrency(worker.hourly_rate || 0)}</p>
              </div>
              <div className="p-4 rounded-lg bg-amber-500/10">
                ...
                <p className="text-xs text-muted-foreground mt-1">{totals.overtimeHours.toFixed(1)} OT hrs × 1.5x</p>
              </div>
              <div className="p-4 rounded-lg bg-success/10">
                <p className="text-sm text-muted-foreground">Total Pay</p>
                <p className="text-xl font-bold text-success">{formatCurrency(totals.totalPay)}</p>
                <p className="text-xs text-muted-foreground mt-1">Per cut-off: {selectedPeriod.label}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                    ...
                    {timesheets.map((ts) => (
                      <TableRow key={ts.id}>
                        <TableCell className="font-medium">
                          {format(new Date(ts.date), 'MMM d')}
                        </TableCell>
                        <TableCell className="text-muted-foreground">
                          {format(new Date(ts.date), 'EEE')}
                        </TableCell>
                        <TableCell>
                          {ts.clock_in_time ? format(new Date(ts.clock_in_time), 'h:mm a') : '—'}
                        </TableCell>
