import { useState, useEffect } from 'react';
import { DashboardLayout } from '@/components/layout/DashboardLayout';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Badge } from '@/components/ui/badge';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { useToast } from '@/hooks/use-toast';
import { supabase } from '@/integrations/supabase/client';
import { KeyRound, Loader2, Search, ShieldCheck, Clock, Mail, Eye, EyeOff, Users, HardHat, UserPlus } from 'lucide-react';
import { format } from 'date-fns';

interface ResetLog {
  id: string;
  user_email: string;
  created_at: string;
  notes: string | null;
}

interface EmployeeOption {
  id: string;
  first_name: string;
  last_name: string;
  email: string;
  position: string | null;
}

interface WorkerOption {
  id: string;
  first_name: string;
  last_name: string;
  email: string | null;
  worker_id: string;
  user_id: string | null;
}

export default function ResetPassword() {
  const [email, setEmail] = useState('');
  const [newPassword, setNewPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [resetLogs, setResetLogs] = useState<ResetLog[]>([]);
  const [logsLoading, setLogsLoading] = useState(true);
  const [employees, setEmployees] = useState<EmployeeOption[]>([]);
  const [selectedEmployeeId, setSelectedEmployeeId] = useState('');
  // Worker tab state
  const [workers, setWorkers] = useState<WorkerOption[]>([]);
  const [selectedWorkerId, setSelectedWorkerId] = useState('');
  const [workerEmail, setWorkerEmail] = useState('');
  const [workerPassword, setWorkerPassword] = useState('');
  const [showWorkerPassword, setShowWorkerPassword] = useState(false);
  const [workerSubmitting, setWorkerSubmitting] = useState(false);
  const { toast } = useToast();

  const selectedWorker = workers.find(w => w.id === selectedWorkerId) ?? null;
  const workerHasAccount = !!selectedWorker?.user_id;

  const fetchLogs = async () => {
    const { data } = await supabase
      .from('password_reset_requests')
      .select('*')
      .order('created_at', { ascending: false })
      .limit(50);
    if (data) setResetLogs(data as ResetLog[]);
    setLogsLoading(false);
  };

  const fetchEmployees = async () => {
    const { data } = await supabase
      .from('employees')
      .select('id, first_name, last_name, email, position')
      .not('user_id', 'is', null)
      .order('first_name', { ascending: true });
    if (data) setEmployees(data);
  };

  const fetchWorkers = async () => {
    const { data } = await supabase
      .from('workers')
      .select('id, first_name, last_name, email, worker_id, user_id')
      .order('first_name', { ascending: true });
    if (data) setWorkers(data as WorkerOption[]);
  };

  useEffect(() => {
    fetchLogs();
    fetchEmployees();
    fetchWorkers();

    const channel = supabase
      .channel('password-reset-logs')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'password_reset_requests' }, () => {
        fetchLogs();
      })
      .subscribe();

    return () => { supabase.removeChannel(channel); };
  }, []);

  const handleEmployeeSelect = (employeeId: string) => {
    setSelectedEmployeeId(employeeId);
    const emp = employees.find(e => e.id === employeeId);
    if (emp) {
      setEmail(emp.email);
    }
  };

  const handleReset = async () => {
    if (!email || !newPassword) {
      toast({ title: 'Error', description: 'Please fill in both email and new password.', variant: 'destructive' });
      return;
    }
    if (newPassword.length < 6) {
      toast({ title: 'Error', description: 'Password must be at least 6 characters.', variant: 'destructive' });
      return;
    }

    setSubmitting(true);
    try {
      const response = await supabase.functions.invoke('admin-reset-password', {
        body: { email, new_password: newPassword },
      });

      if (response.error) throw new Error(response.error.message);
      if (response.data?.error) throw new Error(response.data.error);

      toast({ title: 'Password Reset', description: `Password for ${email} has been reset successfully.` });
      setEmail('');
      setNewPassword('');
      setSelectedEmployeeId('');
    } catch (error: any) {
      toast({ title: 'Error', description: error.message || 'Failed to reset password.', variant: 'destructive' });
    } finally {
      setSubmitting(false);
    }
  };

  const handleWorkerSelect = (workerId: string) => {
    setSelectedWorkerId(workerId);
    const w = workers.find(x => x.id === workerId);
    setWorkerEmail(w?.email ?? '');
  };

  const handleWorkerSubmit = async () => {
    if (!selectedWorker) {
      toast({ title: 'Error', description: 'Please select a worker.', variant: 'destructive' });
      return;
    }
    if (!workerEmail || !workerPassword) {
      toast({ title: 'Error', description: 'Please provide an email and a password.', variant: 'destructive' });
      return;
    }
    if (workerPassword.length < 6) {
      toast({ title: 'Error', description: 'Password must be at least 6 characters.', variant: 'destructive' });
      return;
    }

    setWorkerSubmitting(true);
    try {
      if (workerHasAccount) {
        const response = await supabase.functions.invoke('admin-reset-password', {
          body: { email: workerEmail, new_password: workerPassword },
        });
        if (response.error) throw new Error(response.error.message);
        if (response.data?.error) throw new Error(response.data.error);
        toast({ title: 'Password Reset', description: `Password for ${workerEmail} has been reset successfully.` });
      } else {
        const response = await supabase.functions.invoke('create-worker-account', {
          body: { worker_id: selectedWorker.id, email: workerEmail, password: workerPassword },
        });
        if (response.error) throw new Error(response.error.message);
        if (response.data?.error) throw new Error(response.data.error);
        toast({ title: 'Account Created', description: `Login account created for ${workerEmail}.` });
        await fetchWorkers();
      }
      setSelectedWorkerId('');
      setWorkerEmail('');
      setWorkerPassword('');
    } catch (error: any) {
      toast({ title: 'Error', description: error.message || 'Operation failed.', variant: 'destructive' });
    } finally {
      setWorkerSubmitting(false);
    }
  };

  const filteredLogs = resetLogs.filter(log =>
    log.user_email.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <DashboardLayout title="Reset Password">
      <div className="space-y-6 max-w-4xl mx-auto">
        {/* Reset Form */}
        <Tabs defaultValue="employees" className="space-y-4">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="employees" className="gap-2">
              <Users className="w-4 h-4" /> Employees / Officers
            </TabsTrigger>
            <TabsTrigger value="workers" className="gap-2">
              <HardHat className="w-4 h-4" /> Workers
            </TabsTrigger>
          </TabsList>

          <TabsContent value="employees">
        <Card className="border-0 shadow-lg overflow-hidden">
          <CardHeader className="gradient-primary text-primary-foreground">
            <CardTitle className="flex items-center gap-2">
              <KeyRound className="w-5 h-5" />
              Reset Employee Password
            </CardTitle>
            <CardDescription className="text-primary-foreground/80">
              Select an employee or enter their email to reset their password.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-6 space-y-4">
            {/* Employee Selector */}
            <div className="space-y-2">
              <Label className="flex items-center gap-2">
                <Users className="w-4 h-4 text-muted-foreground" />
                Select Employee
              </Label>
              <Select value={selectedEmployeeId} onValueChange={handleEmployeeSelect}>
                <SelectTrigger>
                  <SelectValue placeholder="Choose an employee..." />
                </SelectTrigger>
                <SelectContent>
                  {employees.map((emp) => (
                    <SelectItem key={emp.id} value={emp.id}>
                      {emp.first_name} {emp.last_name} — {emp.email}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>

            <div className="relative flex items-center gap-3">
              <div className="flex-1 h-px bg-border" />
              <span className="text-xs text-muted-foreground uppercase">or enter email manually</span>
              <div className="flex-1 h-px bg-border" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="email" className="flex items-center gap-2">
                <Mail className="w-4 h-4 text-muted-foreground" />
                User Email
              </Label>
              <Input
                id="email"
                type="email"
                placeholder="user@example.com"
                value={email}
                onChange={(e) => { setEmail(e.target.value); setSelectedEmployeeId(''); }}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="password" className="flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-muted-foreground" />
                New Password
              </Label>
              <div className="relative">
                <Input
                  id="password"
                  type={showPassword ? 'text' : 'password'}
                  placeholder="Minimum 6 characters"
                  value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)}
                  className="pr-10"
                />
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  className="absolute right-0 top-0 h-full px-3"
                  onClick={() => setShowPassword(!showPassword)}
                >
                  {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </Button>
              </div>
            </div>
            <Button
              onClick={handleReset}
              disabled={submitting || !email || !newPassword}
              className="w-full gradient-primary h-12 text-lg"
            >
              {submitting ? (
                <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Resetting...</>
              ) : (
                <><KeyRound className="w-5 h-5 mr-2" /> Reset Password</>
              )}
            </Button>
          </CardContent>
        </Card>
          </TabsContent>

          <TabsContent value="workers">
            <Card className="border-0 shadow-lg overflow-hidden">
              <CardHeader className="gradient-primary text-primary-foreground">
                <CardTitle className="flex items-center gap-2">
                  <HardHat className="w-5 h-5" />
                  Reset Worker Password
                </CardTitle>
                <CardDescription className="text-primary-foreground/80">
                  Reset an existing worker's password, or create a login account for workers who don't have one yet.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    <Users className="w-4 h-4 text-muted-foreground" />
                    Select Worker
                  </Label>
                  <Select value={selectedWorkerId} onValueChange={handleWorkerSelect}>
                    <SelectTrigger>
                      <SelectValue placeholder="Choose a worker..." />
                    </SelectTrigger>
                    <SelectContent>
                      {workers.map((w) => (
                        <SelectItem key={w.id} value={w.id}>
                          {w.first_name} {w.last_name} — {w.worker_id}
                          {w.user_id ? '' : ' (no account)'}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {selectedWorker && (
                    <Badge
                      className={
                        workerHasAccount
                          ? 'bg-success/10 text-success border-success/20'
                          : 'bg-warning/10 text-warning border-warning/20'
                      }
                    >
                      {workerHasAccount ? 'Has login account' : 'No account yet — one will be created'}
                    </Badge>
                  )}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="worker-email" className="flex items-center gap-2">
                    <Mail className="w-4 h-4 text-muted-foreground" />
                    Worker Email
                  </Label>
                  <Input
                    id="worker-email"
                    type="email"
                    placeholder="worker@example.com"
                    value={workerEmail}
                    onChange={(e) => setWorkerEmail(e.target.value)}
                    disabled={workerHasAccount && !!selectedWorker?.email}
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="worker-password" className="flex items-center gap-2">
                    <KeyRound className="w-4 h-4 text-muted-foreground" />
                    {workerHasAccount ? 'New Password' : 'Password'}
                  </Label>
                  <div className="relative">
                    <Input
                      id="worker-password"
                      type={showWorkerPassword ? 'text' : 'password'}
                      placeholder="Minimum 6 characters"
                      value={workerPassword}
                      onChange={(e) => setWorkerPassword(e.target.value)}
                      className="pr-10"
                    />
                    <Button
                      type="button"
                      variant="ghost"
                      size="icon"
                      className="absolute right-0 top-0 h-full px-3"
                      onClick={() => setShowWorkerPassword(!showWorkerPassword)}
                    >
                      {showWorkerPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </Button>
                  </div>
                </div>

                <Button
                  onClick={handleWorkerSubmit}
                  disabled={workerSubmitting || !selectedWorkerId || !workerEmail || !workerPassword}
                  className="w-full gradient-primary h-12 text-lg"
                >
                  {workerSubmitting ? (
                    <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Saving...</>
                  ) : workerHasAccount ? (
                    <><KeyRound className="w-5 h-5 mr-2" /> Reset Worker Password</>
                  ) : (
                    <><UserPlus className="w-5 h-5 mr-2" /> Create Worker Account</>
                  )}
                </Button>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>

        {/* Reset History */}
        <Card className="border-0 shadow-lg">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="flex items-center gap-2">
                  <Clock className="w-5 h-5 text-primary" />
                  Reset History
                </CardTitle>
                <CardDescription>Recent password reset activity</CardDescription>
              </div>
              <div className="relative w-64">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input
                  placeholder="Search by email..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-10"
                />
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {logsLoading ? (
              <div className="p-8 flex items-center justify-center">
                <Loader2 className="w-6 h-6 animate-spin text-primary" />
              </div>
            ) : filteredLogs.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground">
                <ShieldCheck className="w-12 h-12 mx-auto mb-2 opacity-50" />
                <p>No password resets yet</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    <TableHead>User Email</TableHead>
                    <TableHead>Reset Date</TableHead>
                    <TableHead>Status</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredLogs.map((log) => (
                    <TableRow key={log.id}>
                      <TableCell className="font-medium">{log.user_email}</TableCell>
                      <TableCell className="text-muted-foreground">
                        {format(new Date(log.created_at), 'MMM d, yyyy h:mm a')}
                      </TableCell>
                      <TableCell>
                        <Badge className="bg-success/10 text-success border-success/20">Completed</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            )}
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
  );
}
