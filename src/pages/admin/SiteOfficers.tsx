...
import { WeeklyAttendanceDialog } from '@/components/admin/WeeklyAttendanceDialog';
import { CalendarRange } from 'lucide-react';
import { SAOPayrollSheet } from '@/components/admin/SAOPayrollSheet';
import { AssignScheduleDialog } from '@/components/admin/AssignScheduleDialog';
...
export default function SiteOfficers() {
  ...
  const [saoForSchedule, setSaoForSchedule] = useState<SAOEmployee | null>(null);
  const [detailSheetOpen, setDetailSheetOpen] = useState(false);
  const [detailSAO, setDetailSAO] = useState<SAOEmployee | null>(null);
  const [weeklyTimeSAO, setWeeklyTimeSAO] = useState<SAOEmployee | null>(null);
  const [weeklyTimeOpen, setWeeklyTimeOpen] = useState(false);
  const [payrollSAO, setPayrollSAO] = useState<SAOEmployee | null>(null);
  ...
  const fetchWorkerCounts = async () => {
    ...
    const [{ data: workers }, { data: assignments }] = await Promise.all([
      supabase.from('workers').select('id, assigned_sao_id'),
      supabase.from('worker_sao_assignments').select('worker_id, sao_employee_id'),
    ]);
  ...
  return (
    <DashboardLayout title="Site Administration Officers">
      <div className="space-y-6">
        ...
        {/* SAO Cards Carousel */}
        {!loading && filteredSAOs.length > 0 && (
          <div className="px-10">
            <Carousel opts={{ align: 'start', loop: false }} className="w-full">
              <CarouselContent className="-ml-2">
                {filteredSAOs.map((sao) => {
                  const statusDot = sao.status === 'active'
                    ? 'bg-success'
                    : sao.status === 'on_leave'
                      ? 'bg-warning'
                      : 'bg-muted-foreground';
                  return (
                    <CarouselItem key={`card-${sao.id}`} className="pl-2 basis-1/3 sm:basis-1/4 md:basis-1/5 lg:basis-1/6 xl:basis-[12.5%]">
                      <Card
                        className="group cursor-pointer border border-border hover:border-violet-500/40 hover:shadow-xl transition-all duration-300 relative overflow-hidden"
                        onClick={() => { setDetailSAO(sao); setDetailSheetOpen(true); }}
                      >
                        <div className="relative w-full aspect-[3/4] bg-muted overflow-hidden">
                          {sao.photo_url ? (
                            <img
                              src={sao.photo_url}
                              alt={`${sao.first_name} ${sao.last_name}`}
                          ...
                          <div className="absolute top-1 right-1 z-10" onClick={(e) => e.stopPropagation()}>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-44">
                                <DropdownMenuItem onClick={() => { setDetailSAO(sao); setDetailSheetOpen(true); }}>
                                  <Eye className="w-3 h-3 mr-2" /> View Info
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => { setPayrollSAO(sao); setPayrollOpen(true); }}>
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => handleStatusChange(sao.id, sao.status === 'active' ? 'inactive' : 'active')}>
                                  {sao.status === 'active' ? <XCircle className="w-3 h-3 mr-2" /> : <CheckCircle className="w-3 h-3 mr-2" />}
                                  {sao.status === 'active' ? 'Set Inactive' : 'Set Active'}
                                </DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-1.5 text-white">
                            ...
                            <p className="text-[8px] text-white/70 truncate">{sao.position || 'Site Officer'}</p>
                          </div>
                        </div>
                      </Card>
                    </CarouselItem>
                    ...
                    {filteredSAOs.map((sao) => {
                      ...
                      return (
                        <TableRow key={sao.id} className="group hover:bg-violet-500/5 transition-colors">
                          ...
                          <TableCell>
                            <DailyTimeDisplay entityType="employee" entityId={sao.id} showLabel />
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-52">
                                ...
                                <DropdownMenuItem onClick={() => { setSaoForSchedule(sao); setScheduleOpen(true); }}>
        ...
        {detailSAO && (
          <EmployeeDetailSheet
            employee={detailSAO as any}
            open={detailSheetOpen}
            onOpenChange={setDetailSheetOpen}
            onEmployeeUpdated={fetchSAOList}
          />
        )}
        ...
        {weeklyTimeSAO && (
          <WeeklyTimeBreakdown
            ...
            entityType="employee"
            entityId={weeklyTimeSAO.id}
            entityName={`${weeklyTimeSAO.first_name} ${weeklyTimeSAO.last_name}`}
          />
        )}
        ...
        {saoForSchedule && (
          <AssignScheduleDialog
            ...
            saoEmployeeId={saoForSchedule.id}
            saoName={`${saoForSchedule.first_name} ${saoForSchedule.last_name}`}
          />
        )}

        {/* Weekly Attendance Report */}
        <WeeklyAttendanceDialog
          open={showWeeklyReport}
          onOpenChange={setShowWeeklyReport}
          mode="sao"
          people={saoList.map((s) => ({
            id: s.id,
