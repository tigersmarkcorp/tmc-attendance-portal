...
import { Mail, Calendar, Loader2, Key, Eye, EyeOff } from 'lucide-react';
import { format } from 'date-fns';

export default function EncoderProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const { data } = await supabase.from('employees').select('first_name, last_name').eq('id', employeeId).maybeSingle();
      if (data) setProfile({ first_name: data.first_name || '', last_name: data.last_name || '' });
    };
    fetchProfile();
  }, [employeeId]);
  ...
  const getInitials = () => {
    ...
    return user?.email?.substring(0, 2).toUpperCase() || 'EN';
  };

  return (
    <DashboardLayout title="My Profile" portalType="encoder">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 -mt-16">
              ...
              <div className="text-center sm:text-left flex-1 space-y-2">
                <h2 className="text-2xl font-bold text-foreground">
                  {profile.first_name && profile.last_name ? `${profile.first_name} ${profile.last_name}` : 'Encoder'}
                </h2>
