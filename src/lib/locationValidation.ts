...
function evaluateLocationMatch(
  ...
  currentLng: number,
  accuracy: number
): { success: boolean; error?: string; locationName?: string } {
  ...
  for (const loc of locations) {
    ...
    if (distance <= effectiveRadius) {
      return { success: true, locationName: loc.name };
    }
  }
...
export async function checkLocationForWorker(workerId: string): Promise<{ success: boolean; error?: string; locationName?: string }> {
...
export async function checkLocationForEmployee(employeeId: string): Promise<{ success: boolean; error?: string; locationName?: string }> {
  if (!navigator.geolocation) {
    return { success: false, error: 'Geolocation is not supported by your browser' };
  }
