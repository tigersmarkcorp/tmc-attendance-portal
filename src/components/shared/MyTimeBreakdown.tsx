...
import { buildSessions, calculateDayTimeFromSessions, getEntriesForDayPHT, hasCarryOver, getExpandedFetchRange } from '@/lib/overnightTimeCalc';
...
interface MyTimeBreakdownProps {
...
export function MyTimeBreakdown({ entityType, entityId }: MyTimeBreakdownProps) {
  ...
  }, [viewMode, refDate, customRange.from, customRange.to]);
  ...
  const fetchEntries = useCallback(async () => {
    ...
    const { fetchStart, fetchEnd } = getExpandedFetchRange(start, end);
    ...
    setLoading(false);
  }, [entityId, entityType, getRange]);

  useEffect(() => {
    fetchEntries();
    const tableName = entityType === 'employee' ? 'time_entries' : 'worker_time_entries';
    const channel = supabase
      .channel(`my-time-breakdown-${entityType}-${entityId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: tableName }, () => fetchEntries())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [fetchEntries, entityId, entityType]);

  const sessions = buildSessions(entries);
  const getEntriesForDay = (day: Date) => getEntriesForDayPHT(entries, day);

  // Live-updating total: recalculate every second when there's an active session
  const [tick, setTick] = useState(0);
  const hasActiveSession = entries.some((e, i) => {
    if (e.entry_type === 'clock_in' || e.entry_type === 'break_end' || e.entry_type === 'break_start') {
      // Check if there's no clock_out after this entry
  ...
  const totalMs = days.reduce((sum, day) => sum + calculateDayTimeFromSessions(sessions, day), 0);
        ...
        <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
          <CardContent className="p-4 flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-muted-foreground">{days.filter(d => calculateDayTimeFromSessions(sessions, d) > 0).length} days worked</p>
            </div>
          </CardContent>
        </Card>
        ...
        {loading ? (
          ...
          <div className="space-y-1.5">
            {days.map((day) => {
              ...
              const dayMs = calculateDayTimeFromSessions(sessions, day);
              const dayHasCarryOver = hasCarryOver(sessions, day);
                    ...
                    <Card className={`cursor-pointer transition-colors hover:bg-accent/50 ${today ? 'border-primary/40 bg-primary/5' : ''}`}>
                      <CardContent className="p-3 flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {dayHasCarryOver && dayEntries.length === 0 && <p className="text-[10px] text-muted-foreground">overnight</p>}
                        </div>
                      </CardContent>
                    </Card>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    {(dayEntries.length > 0 || dayHasCarryOver) ? (
                      <div className="ml-6 mr-2 mb-2 mt-1 border-l-2 border-primary/20 pl-4 space-y-1.5">
                        {dayHasCarryOver && (
                          <div className="flex items-center justify-between py-1">
                            <span className="text-xs font-medium text-blue-500">🌙 Continued from previous day</span>
                          </div>
                        )}
