...
interface WorkerWorkLocationMapProps {
  workerId: string;
}

export function WorkerWorkLocationMap({ workerId }: WorkerWorkLocationMapProps) {
  ...
  const fetchLocations = async () => {
    ...
    let locationIds = (assignments || []).map((a) => a.location_id);
    ...
    if (locationIds.length === 0) {
      const { data: worker } = await supabase
        ...
        .eq('id', workerId)
        .maybeSingle();
      if (worker?.assigned_location_id) locationIds = [worker.assigned_location_id];
    }
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {locations.length} location{locations.length !== 1 ? 's' : ''} assigned to you
            </CardDescription>
          </div>
