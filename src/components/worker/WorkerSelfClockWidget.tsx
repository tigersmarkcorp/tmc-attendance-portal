...
interface WorkerSelfClockWidgetProps {
  workerId: string;
  workerName?: string;
}

export function WorkerSelfClockWidget({ workerId, workerName }: WorkerSelfClockWidgetProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`worker-clock-widget-${workerId}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'worker_time_entries',
  ...
  const handleAction = async (action: EntryType) => {
    ...
    const locationResult = await checkLocationForWorker(workerId);
  ...
  const handleCapture = async (blob: Blob) => {
    ...
    setSubmitting(true);
    try {
      const fileName = `worker-${workerId}/${pendingAction}-${Date.now()}.jpg`;
      ...
      if (pendingAction === 'clock_in') {
        ...
        if (timesheet?.clock_in_time) {
          ...
          const totalMinutes = Math.floor((phNow.getTime() - clockIn.getTime()) / 60000);
      ...
      console.error('Worker clock action error:', error);
