...
import { Loader2, DollarSign, ChevronLeft, ChevronRight, FileText, Calendar, CheckCircle, Download } from 'lucide-react';
...
export function SAOPayrollSheet({ open, onOpenChange, employee }: SAOPayrollSheetProps) {
  ...
  const handleGeneratePayroll = async () => {
    ...
    setGenerating(true);
    try {
      // Upsert timesheets for each day with work
      for (const d of dayData) {
        ...
        if (existing) {
          ...
          await supabase.from('timesheets').insert(timesheetData);
        }
      }

      toast({
        title: 'Payroll Generated',
        description: `Payroll for ${employee.first_name} ${employee.last_name} has been generated for ${selectedPeriod.label}.`,
      });
    } catch (error: any) {
      toast({
        title: 'Error',
