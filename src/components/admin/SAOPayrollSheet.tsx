...
export function SAOPayrollSheet({ open, onOpenChange, employee }: SAOPayrollSheetProps) {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    const [teRes, lateRes, caRes] = await Promise.all([
      supabase.from('time_entries').select('id, entry_type, timestamp, location').eq('employee_id', employee.id).gte('timestamp', startStr).lte('timestamp', endStr).order('timestamp', { ascending: true }),
    ...
    const { data: la } = await supabase.from('employee_location_assignments').select('location_id').eq('employee_id', employee.id);
    let locIds = (la || []).map((r: any) => r.location_id);
    if (!locIds.length) {
      const { data: e } = await supabase.from('employees').select('assigned_location_id').eq('id', employee.id).maybeSingle();
      if (e?.assigned_location_id) locIds = [e.assigned_location_id];
    }
    if (locIds.length) {
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow className="bg-violet-500/5 font-bold border-t-2 border-violet-500/20">
                        <TableCell colSpan={5} className="text-xs font-bold text-violet-600">TOTAL ({daysWorked} days)</TableCell>
