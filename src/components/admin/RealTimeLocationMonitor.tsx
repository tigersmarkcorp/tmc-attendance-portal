...
interface PersonEntry {
  ...
  role: 'sao' | 'worker';
  hourly_rate: number;
  entries: { entry_type: string; timestamp: string; location: string | null }[];
  /** Actual location name from the person's latest clock_in entry (if recorded) */
...
export function RealTimeLocationMonitor() {
  ...
  const fetchAll = async () => {
    ...
    const [locRes, empAssignRes, workerAssignRes, employeesRes, workersRes, timeEntriesRes, workerTimeEntriesRes, rolesRes] = await Promise.all([
      ...
      supabase.from('time_entries').select('employee_id, entry_type, timestamp, location')
        .gte('timestamp', todayStart).lte('timestamp', todayEnd).order('timestamp', { ascending: true }),
      supabase.from('worker_time_entries').select('worker_id, entry_type, timestamp, location')
        .gte('timestamp', todayStart).lte('timestamp', todayEnd).order('timestamp', { ascending: true }),
    ...
    const empEntriesMap = new Map<string, { entry_type: string; timestamp: string; location: string | null }[]>();
    (timeEntriesRes.data || []).forEach(e => {
      if (!empEntriesMap.has(e.employee_id)) empEntriesMap.set(e.employee_id, []);
      empEntriesMap.get(e.employee_id)!.push({ entry_type: e.entry_type, timestamp: e.timestamp, location: (e as any).location ?? null });
    });

    // Build worker entries map
    const workerEntriesMap = new Map<string, { entry_type: string; timestamp: string; location: string | null }[]>();
    (workerTimeEntriesRes.data || []).forEach(e => {
      if (!workerEntriesMap.has(e.worker_id)) workerEntriesMap.set(e.worker_id, []);
      workerEntriesMap.get(e.worker_id)!.push({ entry_type: e.entry_type, timestamp: e.timestamp, location: (e as any).location ?? null });
    });
    ...
    // Derive the actual clock-in location from a person's latest clock_in entry
    const getActualLocationName = (entries: { entry_type: string; location: string | null }[]): string | null => {
      const clockIns = entries.filter(e => e.entry_type === 'clock_in' && e.location);
      if (clockIns.length === 0) return null;
      return clockIns[clockIns.length - 1].location;
    };
    ...
    const locationData: LocationData[] = locRes.data.map(loc => {
      ...
      empAssignments.forEach(a => {
        ...
        const entries = empEntriesMap.get(emp.id) || [];
        personnel.push({
          id: emp.id,
          ...
          entries,
          actualLocationName: getActualLocationName(entries),
        });
      });
      ...
      workerAssignments.forEach(a => {
        ...
        if (!worker) return;
        const entries = workerEntriesMap.get(worker.id) || [];
        personnel.push({
          id: worker.id,
    ...
    // Re-group personnel under the location where they ACTUALLY clocked in today
    // (fallback: their assigned location). Persons whose actual clock-in location is
    // different from the assigned one move to the correct location card.
    const locByName = new Map<string, LocationData>(
      locationData.map(l => [l.name.trim().toLowerCase(), l])
    );
    const movers: { person: PersonEntry; target: LocationData }[] = [];
    locationData.forEach(loc => {
      loc.personnel = loc.personnel.filter(p => {
        if (!p.actualLocationName) return true; // no recorded location → keep under assigned
        const target = locByName.get(p.actualLocationName.trim().toLowerCase());
        if (!target || target.id === loc.id) return true; // same place or unknown name
        movers.push({ person: p, target });
        return false;
      });
    });
    movers.forEach(({ person, target }) => {
      if (!target.personnel.some(p => p.id === person.id && p.role === person.role)) {
                                  ...
                                  >
                                    ...
                                    {entry.location && (
                                      <span className="ml-0.5 flex items-center">
                                        <MapPin className="w-2.5 h-2.5 mx-0.5" />{entry.location}
                                      </span>
                                    )}
                                  </Badge>
                                ))}
