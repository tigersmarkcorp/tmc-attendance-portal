...
export function AddWorkerForm({ onSuccess, onCancel }: AddWorkerFormProps) {
  ...
  const onSubmit = async (values: WorkerFormValues) => {
    ...
    try {
      ...
      const { data: inserted, error } = await supabase.from('workers').insert({
        worker_id: workerId,
      ...
      // Create login account for the worker
      const { data: acctData, error: acctError } = await supabase.functions.invoke('create-worker-account', {
        body: { worker_id: inserted.id, email: values.email, password: values.password },
      });
      if (acctError || (acctData && acctData.error)) {
        // Rollback worker row so admin can retry
        await supabase.from('workers').delete().eq('id', inserted.id);
        throw new Error(acctData?.error || acctError?.message || 'Failed to create worker login account');
      }

      toast({
        title: 'Worker Added',
        description: `${values.first_name} ${values.last_name} has been added with a login account.`,
      });
      onSuccess();
    } catch (error: any) {
      toast({
        ...
        <Tabs defaultValue="personal" className="w-full">
          ...
          <TabsContent value="personal" className="space-y-4 mt-4">
            ...
            <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
              <p className="text-xs font-semibold text-primary uppercase tracking-wider mb-3">Worker Login Account</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
                  name="email"
              ...
              <p className="text-[10px] text-muted-foreground mt-2">Worker will use these credentials to sign in to the Worker Portal.</p>
            </div>
