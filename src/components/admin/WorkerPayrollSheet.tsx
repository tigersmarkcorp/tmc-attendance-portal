...
import { Loader2, DollarSign, ChevronLeft, ChevronRight, Calendar, CheckCircle, Download, Wallet, MapPin } from 'lucide-react';
...
import { calculateDayPayroll, getPayrollFetchRange } from '@/lib/payrollDayCalc';
...
interface TimeEntry { id: string; entry_type: string; timestamp: string; location?: string | null; }

interface WorkerPayrollSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  worker: { id: string; first_name: string; last_name: string; hourly_rate: number | null; } | null;
}
...
export function WorkerPayrollSheet({ open, onOpenChange, worker }: WorkerPayrollSheetProps) {
  ...
  const fetchEntries = useCallback(async () => {
    if (!worker) return;
    setLoading(true);
    // Expand fetch range by a day on each side so overnight sessions are complete
    const { startStr, endStr } = getPayrollFetchRange(selectedPeriod.start, selectedPeriod.end);
    ...
    const [teRes, lateRes, caRes] = await Promise.all([
      supabase.from('worker_time_entries').select('id, entry_type, timestamp, location').eq('worker_id', worker.id).gte('timestamp', startStr).lte('timestamp', endStr).order('timestamp', { ascending: true }),
      (supabase as any).from('payroll_late_minutes').select('date, minutes').eq('worker_id', worker.id).gte('date', startDate).lte('date', endDate),
      (supabase as any).from('cash_advances').select('id, description, amount, created_at').eq('worker_id', worker.id).eq('period_start', startDate).eq('period_end', endDate).order('created_at', { ascending: false }),
    ]);

    if (teRes.data) setEntries(teRes.data);

    // Fallback location names (for entries recorded before location tracking)
    const { data: la } = await supabase.from('worker_location_assignments').select('location_id').eq('worker_id', worker.id);
    let locIds = (la || []).map((r: any) => r.location_id);
    if (!locIds.length) {
      const { data: w } = await supabase.from('workers').select('assigned_location_id').eq('id', worker.id).maybeSingle();
      if (w?.assigned_location_id) locIds = [w.assigned_location_id];
    }
    if (locIds.length) {
      const { data: locs } = await supabase.from('work_locations').select('name').in('id', locIds);
      setAssignedLocations((locs || []).map((l: any) => l.name));
    } else {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`worker-payroll-${worker.id}-${selectedPeriodIndex}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'worker_time_entries', filter: `worker_id=eq.${worker.id}` }, () => fetchEntries())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'cash_advances', filter: `worker_id=eq.${worker.id}` }, () => fetchEntries())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'payroll_late_minutes', filter: `worker_id=eq.${worker.id}` }, () => fetchEntries())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [open, worker, selectedPeriodIndex, fetchEntries]);

  const dayData: DayPayrollData[] = days.map(day => {
    const dateStr = format(day, 'yyyy-MM-dd');
    const calc = calculateDayPayroll(entries, day, regularHoursPerDay);
    ...
    return {
      ...
      timeInDate: calc.timeInDate,
      timeOutDate: calc.timeOutDate,
      locationIn: calc.locationIn,
      locationOut: calc.locationOut,
      carriedIn: calc.carriedIn,
      carriedOut: calc.carriedOut,
  ...
  const grossOvertime = totalOvertime * hourlyRate; // OT = hourly rate
  ...
  const saveLateMinutes = async (dateStr: string, rawValue: string) => {
    ...
    const payload: any = { worker_id: worker.id, date: dateStr, minutes: parsed };
  ...
  const handleGeneratePayroll = async () => {
    ...
    try {
      for (const d of dayData) {
        ...
        const otPay = d.overtimeHours * hourlyRate;
        const dayTotalPay = regPay + otPay - d.lateDeduction;

        const { data: existing } = await supabase.from('worker_timesheets').select('id').eq('worker_id', worker.id).eq('date', dateStr).maybeSingle();
        const timesheetData = {
          worker_id: worker.id,
          date: dateStr,
        ...
        if (existing) await supabase.from('worker_timesheets').update(timesheetData).eq('id', existing.id);
        else await supabase.from('worker_timesheets').insert(timesheetData);
      }
      toast({ title: 'Payroll Generated', description: `Payroll for ${worker.first_name} ${worker.last_name} — ${selectedPeriod.label}. Net: ${formatCurrency(totalPay)}` });
    } catch (error: any) {
      ...
      <SheetContent className="w-full sm:max-w-5xl overflow-y-auto">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex justify-end gap-2 flex-wrap">
            ...
            <Button onClick={handleGeneratePayroll} disabled={generating || daysWorked === 0} className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 gap-2">
                  ...
                  <Table>
                    ...
                    <TableBody>
                          ...
                          <TableRow key={dateStr} className={isSatSun ? 'bg-muted/30' : ''}>
                              ...
                              {d.timeIn ? (
                                <div className="flex flex-col leading-tight">
                                  ...
                                  <span className="text-[10px] text-muted-foreground">
                                    {format(new Date(d.timeInDate + 'T00:00:00'), 'EEE, MMM d, yyyy')}
                                  </span>
                                </div>
                              ) : '—'}
                            </TableCell>
                            <TableCell className="text-xs font-mono">
                              {d.timeOut ? (
                                <div className="flex flex-col leading-tight">
                                  <span>{d.timeOut}</span>
                                  <span className={`text-[10px] ${d.timeOutDate !== format(d.date, 'yyyy-MM-dd') ? 'text-warning' : 'text-muted-foreground'}`}>
                                    {format(new Date(d.timeOutDate + 'T00:00:00'), 'EEE, MMM d, yyyy')}
                                  </span>
                                </div>
                              ) : '—'}
                            </TableCell>
                            <TableCell className="text-xs">
                              {d.locationIn || d.locationOut ? (
                                <div className="flex flex-col gap-0.5 min-w-[120px]">
                                  {d.locationIn && (
                                    <span className="flex items-center gap-1 text-[11px] text-success">
                                      <MapPin className="w-3 h-3 shrink-0" />In: {d.locationIn}
                                    </span>
                                  )}
                                  {d.locationOut && d.locationOut !== d.locationIn && (
                                    <span className="flex items-center gap-1 text-[11px] text-destructive">
                                      <MapPin className="w-3 h-3 shrink-0" />Out: {d.locationOut}
                                    </span>
                                  )}
                                </div>
                              ) : d.totalHoursMs > 0 && assignedLocations.length > 0 ? (
                      ...
                      <TableRow className="bg-amber-500/5 font-bold border-t-2 border-amber-500/20">
                        <TableCell colSpan={5} className="text-xs font-bold text-amber-600">TOTAL ({daysWorked} days)</TableCell>
                      ...
                      {totalCashAdvance > 0 && (
                        <TableRow className="bg-orange-500/5">
                          <TableCell colSpan={10} className="text-xs font-semibold text-orange-600">Cash Advance Deductions ({cashAdvances.length})</TableCell>
                      ...
                      <TableRow className="bg-primary/10 border-t-2 border-primary/30">
                        <TableCell colSpan={10} className="text-sm font-bold text-primary">NET TOTAL PAY</TableCell>
        ...
        <CashAdvanceDialog
          ...
          subjectType="worker"
          subjectId={worker.id}
          subjectName={`${worker.first_name} ${worker.last_name}`}
