...
export function WorkerPayrollSheet({ open, onOpenChange, worker }: WorkerPayrollSheetProps) {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    const [teRes, lateRes, caRes] = await Promise.all([
      supabase.from('worker_time_entries').select('id, entry_type, timestamp').eq('worker_id', worker.id).gte('timestamp', startStr).lte('timestamp', endStr).order('timestamp', { ascending: true }),
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow className="bg-amber-500/5 font-bold border-t-2 border-amber-500/20">
                        <TableCell colSpan={4} className="text-xs font-bold text-amber-600">TOTAL ({daysWorked} days)</TableCell>
