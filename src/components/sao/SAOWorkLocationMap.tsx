...
interface SAOWorkLocationMapProps {
  employeeId: string;
}

export function SAOWorkLocationMap({ employeeId }: SAOWorkLocationMapProps) {
  ...
  const fetchLocations = async () => {
    // Get the SAO's assigned location IDs via employee_location_assignments
    ...
    // Fetch only those locations
    const { data: locData } = await supabase
      .from('work_locations')
      .select('*')
      .in('id', locationIds);

    // Count workers assigned to these locations
    ...
    if (locData) {
      ...
      workerAssignments?.forEach(a => countMap.set(a.location_id, (countMap.get(a.location_id) || 0) + 1));

      setLocations(locData.map(loc => ({
        ...loc,
        worker_count: countMap.get(loc.id) || 0,
      })));
    }
    setLoading(false);
  };
      ...
      marker.bindPopup(`
        <div style="min-width:180px">
          ...
          <div style="margin-top:8px;font-size:12px">
            ...
            <p style="margin:2px 0">👥 Workers: ${loc.worker_count}</p>
