...
import { getPhilippineNow, getPhilippineToday, formatPHTime, getPhilippineTodayStart, getPhilippineTodayEnd } from '@/lib/philippineTime';
...
type ActionType = 'clock_in' | 'clock_out' | 'break_start' | 'break_end';
 ...
 export function WorkerClockWidget({ worker, saoId, onClockAction }: WorkerClockWidgetProps) {
  ...
  const checkLocation = async (): Promise<{ success: boolean; error?: string; locationName?: string }> => {
   ...
   const startCamera = async (type: ActionType) => {
     ...
     setVerifiedLocationName(locationResult.locationName || null);
     ...
     try {
       ...
       if (actionType === 'clock_in') {
         await supabase.from('worker_timesheets').upsert({
           worker_id: worker.id,
           date: phToday,
           clock_in_time: phNow.toISOString(),
           status: 'pending'
         }, { onConflict: 'worker_id,date' });
       } else if (actionType === 'clock_out') {
         const { data: timesheet } = await supabase.from('worker_timesheets').select('*').eq('worker_id', worker.id).eq('date', phToday).single();
         if (timesheet?.clock_in_time) {
           const clockInTime = new Date(timesheet.clock_in_time);
           const totalMinutes = Math.floor((phNow.getTime() - clockInTime.getTime()) / 60000);
           const totalHours = totalMinutes / 60;
           await supabase.from('worker_timesheets').update({
             clock_out_time: phNow.toISOString(),
         ...
         <DialogContent className="w-full max-w-full h-[100dvh] sm:max-w-2xl sm:h-auto sm:max-h-[95vh] p-4 sm:p-6 overflow-y-auto">
           ...
           <div className="flex items-center gap-3 p-3 rounded-lg bg-muted mb-2">
             ...
             <Badge variant="outline" className="shrink-0">
               <Clock className="w-3 h-3 mr-1" />{formatPHTime(new Date())}
             </Badge>
           </div>
