...
type PortalType = 'admin' | 'employee' | 'sao' | 'encoder';
...
const adminNavGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
    { label: 'Payroll Reports', href: '/admin/payroll', icon: <span className="w-5 h-5 flex items-center justify-center">₱</span> }
    ],
  },
  {
    title: 'System',
...
export function DashboardLayout({ children, title, portalType }: DashboardLayoutProps) {
  ...
  const currentPortal = portalType || (role === 'admin' ? 'admin' : role === 'site_admin_officer' ? 'sao' : role === 'encoder' ? 'encoder' : 'employee');
  const navGroups = currentPortal === 'admin' ? adminNavGroups : currentPortal === 'sao' ? saoNavGroups : currentPortal === 'encoder' ? encoderNavGroups : employeeNavGroups;
  ...
  const handleSignOut = async () => {
    await signOut();
    const loginPath = currentPortal === 'admin' ? '/admin/login' : currentPortal === 'sao' ? '/sao/login' : currentPortal === 'encoder' ? '/encoder/login' : '/employee/login';
    navigate(loginPath);
  };
  ...
  const isActiveRoute = (href: string) => {
    if (href === '/admin' || href === '/employee' || href === '/sao' || href === '/encoder') {
        ...
        >
          ...
          <div className={cn(
            ...
            <Link to="/" className="flex items-center gap-3">
              ...
              {!collapsed && (
                <span className="text-lg font-bold text-sidebar-foreground">TMC Time Pro</span>
              )}
            </Link>
            <button
              onClick={() => setSidebarOpen(false)}
          ...
          {!collapsed && (
            <div className="px-4 py-3 border-b border-sidebar-border">
              ...
              >
                {currentPortal === 'admin' ? '⚡ Administrator' : currentPortal === 'sao' ? '🛡️ Site Admin Officer' : currentPortal === 'encoder' ? '⌨️ Encoder' : '👤 Employee'}
              </Badge>
            </div>
          )}
              ...
              {title && (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground hidden sm:block">
                    {currentPortal === 'admin' ? 'Admin Panel' : currentPortal === 'sao' ? 'SAO Portal' : currentPortal === 'encoder' ? 'Encoder Portal' : 'Employee Portal'}
                  </p>
                </div>
              )}
            </div>
